"""
Extract activations from DINOv2 Vision Transformer and save to computer. These activations (otherwise called patches) will 
be fed into the SAE model. 
The way we do this is we define a wrapper that accesses DinoV2, getting the module blocks of the module and simulating the 
forward pass. We then create an activation recorder wrapper that takes in the ViT model wrapper, and saves the intermediate 
patches that are passed through the model at each block, specifically obtaining the patches at the layer we specify. These
are the patches we use for the SAE.
"""
import warnings
...
#CONFIG 
DATA_ROOT = "/Users/danielpanariti/Documents/universite/m2_2025-2026_MIND/s3/deep_l/projet/BIOSCAN-5M/bioscan5m/images/cropped_256"
...
D_VIT = 768
N_PATCHES = 256 # 
LAYERS_TO_EXTRACT = [-2]  # List of layers that we extract from, we take second to last as in the paper
IMGS_PER_SHARD = 10000 # how many images we put the patches of into the .bin file
...
class ActivationRecorder(nn.Module):
    """Records activations from multiple layers. This uses the register_forward_hook function in order to
    capture the patches at the second to last layer, this runs only on the layer index we provide it (-2).
    """
...
def get_transform():
    return transforms.Compose([
        ...
        transforms.Normalize(
            mean=[0.485, 0.456, 0.406], # typical for DiNO (à vérifier...)
            std=[0.229, 0.224, 0.225]
        ),
    ])


# MAIN: EXTRACTION OF ACTIVATIONS
...
def save_shard(activations_list, shard_idx, shape):
    """Save activations to a .bin file. """
